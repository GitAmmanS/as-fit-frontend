    ...
    <div className='flex items-center justify-evenly py-5  font-medium '>
       <Link to='/'> <img src={assets.asfit} className='w-18 ' alt='no pic'/> </Link>
        ...
        <div className='flex items-center gap-6'>
            <img onClick={()=>setShowSearch(true)} src={assets.search_icon} className='w-5 cursor-pointer ' alt='no search image '/>

            <div className="group relative">
           <Link to='/login'> <img src={assets.profile_icon} className='w-5 cursor-pointer ' alt='no profile image'/> </Link>
            ...
            <Link to='/cart' className='relative'>
                <img src={assets.cart_icon} alt="no cart" className='w-5 min-w-5' />
            ...
            <img onClick={()=>setVisible(true)} src={assets.menu_icon} alt="no icon" className='w-5 cursor-pointer sm:hidden' />
        </div>
        {/* sidebar menu for small screen */}
        <div className={`absolute top-0 text-black right-0 bottom-0 overflow-hidden bg-white transition-all ${visible? 'w-full': 'w-0'}`}>
